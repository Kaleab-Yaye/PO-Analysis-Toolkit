...
# Introduction
In this write-up, I will demonstrate how I was able to defeat the sophisticated 3rd-party anti-fraud SDK the app uses to check if the device is rooted, if it is an emulator, or if `Magisk` is present on the system. In general, this is the center where anti-fraud checks happen, and defeating this SDK results in app-wide vulnerability for various attacks. In this write-up, I will show how I achieved it.

## First encounter with `ishumei` SDK
The `ishumei` SDK is a commercial-grade, Chinese anti-fraud SDK that Poppo Live devs integrated into their app. As you can see from the image below, `ishumei` is made and meant to be hostile to anyone who tries to reverse-engineer and understand the inner workings of the SDK.

<img width="1183" height="516" alt="image" src="https://github.com/user-attachments/assets/a01471ed-e8a0-48a9-af89-ccb8bbc55005" />

Going into this SDK raw with all this obfuscation will feel like slamming into concrete. So I had to improvise.

## How is this SDK used by the app?
As I said, to defeat this SDK I can't go through the front door; that is a way which is too hostile and probably leads to a war I couldn't win (the SDK creators obfuscated it for this exact reason). Instead, I had to see how the app uses this SDK.

So I designed a script that searches for any text that resembles the pattern `Lcom/ishumei/`, through the decompiled `smali` files of the app, excluding calls from inside the `ishumei` SDK.
...
I expected this SDK to be called from many classes of the app, but the results clearly showed the calls originate only from one part of the app: `Lcom/androidtool/CommonApp` and inside this class, the method calling the SDK is `.method private final initSmSdk()V`.
<img width="1916" height="1004" alt="image" src="https://github.com/user-attachments/assets/eb548afe-cecc-4862-b27d-8e1706f4b96c" />


## Inside the method `initSmSdk()V`

<img width="1876" height="908" alt="image" src="https://github.com/user-attachments/assets/461179f9-fe8d-49fc-909e-40f20657bbb2" />

This method is how the app talks to and instantiates the SDK. There are a lot of things that happen in this method, but most are configurations for the SDK. The two most important lines in this method for our analysis are:
...
What this means is that the complex and sophisticated checks done by the `ishumei` SDK boil down to those few classes. Now we could potentially start our tracing within those classes and methods, but since we are planning to also bypass the whole thing dynamically, dealing with how the app works with what is passed to the implementation (`onError` and `onSuccess`) suffices for now.


## How the app implemented the callback
We will start with `onSuccess(Ljava/lang/String;)V`. The reason is that `onError`, while the SDK creators made it to be more than just an outlet for logging errors, in this app it is just what `onError` is doing.
...
```terminal
C:\RE\PO-RE\PO\smali_classes3\com\androidtool\CommonApp.smali:1650: invoke-virtual {v1}, Lcom/androidtool/common/data/PreferenceManager;->getSmeiId()Ljava/lang/String;
C:\RE\PO-RE\PO\smali_classes3\com\androidtool\CommonApp.smali:1720: invoke-virtual {v0}, Lcom/androidtool/common/data/PreferenceManager;->getSmeiId()Ljava/lang/String;
```
